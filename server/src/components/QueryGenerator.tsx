...
import { FaSearch, FaArrowRight, FaCheck, FaList, FaDownload, FaFile, FaChartBar, FaFileAlt, FaTrash, FaUnlock, FaExchangeAlt, FaProjectDiagram } from 'react-icons/fa';
...
const QueryGenerator: React.FC<QueryGeneratorProps> = ({ initialData, onSaveQuery, savedQueries, onClearQueries }) => {
  ...
  const handleGetSynonyms = async () => {
    try {
      ...
      if (Array.isArray(response.data.synonyms)) {
  ...
  const handleSynonymClick = (synonym: string) => {
    setPubMedQuery(prevQuery => prevQuery + ' ' + synonym);
  };
      ...
      case 4:
        return (
          <div>
            ...
            />
            <SynonymList synonyms={synonyms} onSynonymClick={handleSynonymClick} onGetSynonyms={handleGetSynonyms} />
  ...
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6 text-gray-800">Query Generator</h1>
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h2 className="text-xl font-semibold mb-4 text-gray-700">Que souhaitez-vous faire ?</h2>
        <div className="flex flex-col space-y-4">
          <div className="relative">
            ...
            <button
              onClick={handleNextStep}
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-teal-500 text-white px-4 py-1 rounded-full hover:bg-teal-600 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
            >
              <FaArrowRight />
            </button>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <button
              onClick={() => setStep(1)}
              className="flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500"
            >
              <FaSearch className="mr-2" /> Nouvelle query
            </button>
            <button
              onClick={() => setStep(4)}
              className="flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500"
            ...
            <button
              ...
              className="flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500"
            >
              <FaFileAlt className="mr-2" /> File screening
            </button>
            <button
              onClick={() => setStep(5)}
              className="flex items-center justify-center px-4 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-teal-500"
            >
              <FaProjectDiagram className="mr-2" /> PRISM diagram
            </button>
          </div>
        </div>
      </div>
